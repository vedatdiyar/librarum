    ...
    <section className="space-y-10 pb-20">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={`ai-stat-skel-client-${i}`} className="glass-panel flex items-center gap-4 rounded-3xl border-white/5 bg-white/1 p-6">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        {Array.from({ length: 4 }).map((_, i) => (
          <div className="space-y-8 rounded-[32px] border border-white/5 bg-white/1 p-8" key={`ai-section-skel-client-${i}`}>
             ...
             <div className="space-y-4">
               {Array.from({ length: 2 }).map((_, j) => (
                 <Skeleton className="h-32 w-full rounded-2xl border border-white/5 bg-white/2" key={`ai-item-skel-client-${i}-${j}`} />
               ))}
             </div>
          </div>
        ))}
