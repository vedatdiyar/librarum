...
import { LoaderCircle, Hash, ScanSearch, Search, BookOpen } from "lucide-react";
...
import type { AuthorOption, PublisherOption } from "@/types";
    ...
    <div className="group/field flex flex-col gap-3">
      <div className="flex flex-col gap-1.5">
        <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 px-1">
          <LabelTag className="text-[10px] font-bold tracking-[0.3em] text-foreground uppercase transition-colors group-focus-within/field:text-primary" htmlFor={id}>
            {label}
          </LabelTag>
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          {[
            ...
            },
            {
              ...
              label: "ALT BAŞLIK",
              description: "Kitabın alt başlığı veya cilt bilgisi.",
...
function IsbnMetadataField({
  ...
  const handleMobileBarcodeDetected = React.useCallback(
    (isbn: string) => {
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <MobileBarcodeScanner
          ...
          onDetected={handleMobileBarcodeDetected}
        />
      </div>
      ...
      <div className="glass-panel space-y-6 rounded-3xl border-white/5 bg-white/1 p-8 transition-all group-hover:border-white/10">
        ...
        {draftAuthorNames.length > 0 ? (
          <SelectionPills
            ...
            onEdit={(id, newName) => {
              const oldName = id.replace(/^draft:/, "");
              updateDraftAuthorName(oldName, newName);
            }}
            onRemove={(id) => {
...
function PublisherYearPageSection({
  publishers,
  publisherQuery,
  setPublisherQuery,
  canCreatePublisher,
  ...
  const { register, formState: { errors }, setValue, watch } = useFormContext();
  const selectedPublisher = watch("publisher") as { id?: string; name?: string } | null;
  const selectedPublisherName = selectedPublisher
    ? ("id" in selectedPublisher
        ? publishers.find((publisher) => publisher.id === selectedPublisher.id)?.name ?? "Seçili yayınevi"
        : selectedPublisher.name ?? "Seçili yayınevi")
    : null;
      ...
      >
        <div className="flex flex-col gap-3">
          <div className="relative flex-1">
            <Search className="absolute top-1/2 left-4 h-4 w-4 -translate-y-1/2 text-foreground" />
            <input
              aria-label="Yayınevi Ara"
              className="h-12 w-full rounded-xl border border-white/5 bg-white/2 pr-4 pl-12 text-sm text-white shadow-inner transition-all outline-none placeholder:text-foreground hover:bg-white/4 focus:border-primary/40 focus:bg-white/8"
              id="publisherQuery"
              name="publisherQuery"
              onChange={(event) => setPublisherQuery(event.target.value)}
              placeholder={selectedPublisher ? ("id" in selectedPublisher ? "Seçili yayınevini değiştir..." : selectedPublisher.name) : "Yayın evi ara"}
              value={publisherQuery}
            />
          </div>
          {selectedPublisherName && !publisherQuery && "id" in (selectedPublisher ?? {}) ? (
            <p className="rounded-xl border border-primary/20 bg-primary/5 px-3 py-2 text-xs text-primary">
              Seçili yayınevi: <span className="font-semibold">{selectedPublisherName}</span>
            </p>
          ) : null}
              ...
              <div className="flex flex-wrap gap-2">
                {publishers.slice(0, 5).map((pub) => (
                  <button
                    ...
                    className="rounded-full border border-white/10 bg-white/4 px-3 py-1.5 text-xs text-white transition-colors hover:border-primary/30 hover:bg-primary/10"
                    onClick={() => {
                      setValue("publisher", { id: pub.id }, { shouldDirty: true, shouldValidate: true });
                      setPublisherQuery("");
                    }}
                    type="button"
                  >
                ...
                {canCreatePublisher ? (
                  <button
                    ...
                    onClick={() => void createPublisher(publisherQuery.trim())}
                    type="button"
                  >
                    Yeni Ekle: &quot;{publisherQuery}&quot;
                  </button>
                ) : null}
              </div>
            </div>
